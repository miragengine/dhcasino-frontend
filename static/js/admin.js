...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function reloadAdminPanel() {
        fetchAndRender("https://dhcasino.fly.dev/admin/api/connected", connectedTable, ["Username"], (name) =>
            `<tr><td>${name}</td></tr>`
        );

        fetchAndRender("https://dhcasino.fly.dev/admin/api/active-players", playingTable, ["Username"], (name) =>
            `<tr><td>${name}</td></tr>`
        );

        fetch("https://dhcasino.fly.dev/admin/api/logs")
            .then(res => res.json())
            .then(data => {
                if (!logsTable) return;
            ...
            fetch("https://dhcasino.fly.dev/admin/api/users")
            .then(res => res.json())
            .then(users => {
                fetch("https://dhcasino.fly.dev/admin/api/logs")
                    .then(res => res.json())
    ...
    playerSearch.addEventListener("input", async () => {
        ...
        const res = await fetch(`https://dhcasino.fly.dev/admin/api/player/${username}`);
        const data = await res.json();
        if (data.error) {
    ...
    document.addEventListener("click", async (e) => {
        if (e.target.classList.contains("fund-submit-btn")) {
            ...
            const res = await fetch("https://dhcasino.fly.dev/admin/api/change-balance", {
                method: "POST",
